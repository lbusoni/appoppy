...
from appoppy.petalometer import Petalometer, PetalComputer
...
def test_of_tip_measurement_from_ciao_ciao_with_petaled_subaperture(
        tip_coeff_in_um=2.2):
    '''
    The OPD reconstructed from the CiaoCiao WFS is unwrapped.
    Tip is measured from each of the 12 overlapped sectors and 
    '''
    rot_angle = 20
    pet = Petalometer(rotation_angle=rot_angle,
                      zernike=np.array([0, tip_coeff_in_um]) * u.um,
                      should_unwrap=True)
    pet.sense_wavefront_jumps()
    unwrapped_opd = pet.reconstructed_phase
    angs = (90, 90 - rot_angle, 30, 30 - rot_angle, -30, -30 - rot_angle, -
                90, -90 - rot_angle, -150, -150 - rot_angle, -210,
                -210 - rot_angle, -270)
    ifm_on_petals = []
    for i in range(len(angs) - 1):
        ifm = PetalComputer._mask_ifgram(unwrapped_opd, (angs[i + 1], angs[i]))
        ifm_on_petals.append(ifm)
    return unwrapped_opd, ifm_on_petals


def test_of_tip_measurement_from_ciao_ciao_with_circular_subaperture(
        tip_coeff_in_um=2.2):
    '''
...
def test_of_tip_measurement_from_ciaociao(tip_coeff_in_um=2.2):
    '''
