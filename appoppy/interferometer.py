...
class BaseInterferometer(Snapshotable):
    '''
    Returns the output of a simple interferometer - not implementing phase shifting techniques.
    The phase difference is reconstructed from the following equation:

        I = I1 + I2 + 2 sqrt(I1*I2) * cos(delta_phi)

    thus, it ranges from 0 to pi.
    ...
    ----------
        optical_system_1: poppy.OpticalSystem
           Optical system for reference beam. 

        optical_system_2: poppy.OpticalSystem
            Optical system to be analyzed.
    ...
    def acquire(self):
        ...
        A = self._os1.pupil_intensity() + self._os2.pupil_intensity()
        B = 2 * np.sqrt(self._os1.pupil_intensity() * self._os2.pupil_intensity()) 
        self._wf_0 = self.combined_wavefront
...
class PhaseShiftInterferometer(BaseInterferometer):
    '''
    Implement the acquisition of an interferometer reconstructing the phase by means of a
    4-step shifting technique.
